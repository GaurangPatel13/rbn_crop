...
import { getRankHistoryDU, updateMQDURank } from '../../api/admin-api';
...
const DUC = () => {
 ...
 const handleSearch = async () => {
    ...
    try {
      const response = await getRankHistoryDU(queryString);
      await fetchData();
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await getRankHistoryDU();
      setData(res?.data);
    } catch (error) {
      console.log(error)
    } finally {
...
useEffect(() => {
  ...
  const renderRow = (item, index) => (
   ...
   const handleAction = async (id, status) => {
      try {
        setLoading(true);
        await updateMQDURank(id, status);
        Swal.fire({
          position: 'center',
          icon: 'success',
